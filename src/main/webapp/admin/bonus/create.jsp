    ...
    <form method="post" action="${pageContext.request.contextPath}/admin/bonus/create">
      ...
      <div>
        ...
        <select id="type" name="type" required>
          ...
          <option value="LONGEVITY">Permanencia</option>
          <option value="UNIFORM">Vestuario</option>
          <option value="OTHER">Otro</option>
        </select>
      </div>
      <div>
        <label for="annualAmount">Importe anual (€)</label><br/>
        <input id="annualAmount" name="annualAmount" type="number" step="0.01" min="0" required/>
      </div>
      <div>
        <label for="requiredMonths">Meses requeridos (solo permanencia)</label><br/>
        <input id="requiredMonths" name="requiredMonths" type="number" min="0" placeholder="0"/>
      </div>
      <div>
        <span>Meses de pago:</span><br/>
        <c:set var="meses" value="Ene,Feb,Mar,Abr,May,Jun,Jul,Ago,Sep,Oct,Nov,Dic"/>
        <c:forEach var="i" begin="0" end="11">
          <label style="margin-right:8px;"><input type="checkbox" name="paidMonths" value="${i}" checked/> ${fn:split(meses,',')[i]}</label>
        </c:forEach>
      </div>
      <div>
      ...
      <div>
        <button type="submit" class="btn" style="padding:8px 12px; background:#0a66c2; color:#fff; border-radius:6px; border:none;">Guardar</button>
        <a class="btn" href="${pageContext.request.contextPath}/admin/agreements/view?id=${param.agreementId}" style="margin-left:8px;">Cancelar</a>
      </div>
    </form>
  </div>
</body>
